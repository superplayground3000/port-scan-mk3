...
- **Rich-mode auto-detect** (`pkg/input/header_match.go`, `rich_types.go`): triggered when all 10 headers present (in order): `src_ip,src_network_segment,dst_ip,dst_network_segment,service_label,protocol,port,decision,matched_policy_id,reason`. In rich mode `-cidr-ip-col`/`-cidr-ip-cidr-col` are ignored; only `decision=accept` + `protocol=tcp` rows are scanned. Basic-mode required headers default `ip`,`ip_cidr`.
